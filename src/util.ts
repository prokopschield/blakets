...
export function normalizeInput (input: string | { buffer: ArrayBuffer }): Uint8Array {
...
export function toHex (bytes: ArrayLike<number>): string {
	return Array.prototype.map.call(bytes, function (n: number) {
...
export function debugPrint (label: string, arr: ArrayLike<number>, size: number) {
	var msg = '\n' + label + ' = '
